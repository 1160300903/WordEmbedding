...
def cps2cnt(src_file, output_file):
    ...
    with open(output_count,"w") as output:
        for word in counts:
            for context in counts[word]:
                output.write(word+" "+context+" "+str(counts[word][context])+"\n")

if __name__ == "__main__":
    corpus = st.CPR_DIR + sys.argv[1] if len(sys.argv) > 1 else st.CPR_DIR + "mono.tok.de"

    output_count = st.CNT_DIR + "F" + str(st.WORD_FREQ) + "-W" + str(st.WINDOW_LENGTH) + "."
    output_count += sys.argv[2] if len(sys.argv) > 2 else "1de"
