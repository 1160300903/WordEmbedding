...
def corpus2vocab(src_file, trg_file, src_count, trg_count, output_src, output_trg):
    ...
    trg_word2index = {}
    assign_index(trg_file, trg_count, trg_word2index, bias = len(src_word2index))
...
if __name__ =="__main__":
    ...
    src_count = st.CNT_DIR + sys.argv[3] if len(sys.argv[3]) > 3 else st.CNT_DIR + ""
    trg_count = st.CNT_DIR + sys.argv[4] if len(sys.argv[4]) > 4 else st.CNT_DIR + ""

    para = src_count.split("/")[-1].split(".")[0]
    output_src = st.VOCAB_DIR + para + "."
    output_src += sys.argv[5] if len(sys.argv) > 5 else ""
    
    output_trg = st.VOCAB_DIR + para + "."
    output_trg += sys.argv[6] if len(sys.argv) > 6 else ""
