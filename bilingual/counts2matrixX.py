...
if __name__ == "__main__":
    src_file = st.CNT_DIR + sys.argv[1] if len(sys.argv) > 1 else st.CNT_DIR + "F10-W5.2en"
    trg_file = st.CNT_DIR + sys.argv[2] if len(sys.argv) > 2 else st.CNT_DIR + "F10-W5.2zh"
    src_vocab = st.VOCAB_DIR + sys.argv[3] if len(sys.argv) > 3 else st.VOCAB_DIR + "F10-W5.2en"
    trg_vocab = st.VOCAB_DIR + sys.argv[4] if len(sys.argv) > 4 else st.VOCAB_DIR + "F10-W5.2zh"
    
    para = src_file.split("/")[-1].split(".")[0]
    output_file = st.MATRIX_DIR + para + "." + sys.argv[5] if len(sys.argv) > 5 else st.MATRIX_DIR + para + "." + "X-en-zh"
