...
/**************************************************************
 * multiplication of matrix B by vector x, where B = DX.TD.T DXD.T,     *
 * and D and X are nrow by ncol (nrow >> ncol). Hence, B is of order *
 * n = ncol (y stores product vector).		              *
 **************************************************************/
void svd_opb(SMat matX, SMat matD, double *x, double *y, double *temp) {
  long i, j, end;
  long *pointrX = matX->pointr, *rowindX = matX->rowind;
  double *valueX = matX->value;

  long *pointrD = matD->pointr, *rowindD = matD->rowind;
  double *valueD = matD->value;
  long n = matD->rows;

  SVDCount[SVD_MXV] += 2;
  
  for (i = 0; i < matD->rows; i++) temp[i] = 0.0;
  
  for (i = 0; i < matD->cols; i++) {//temp = D.T*x
    end = pointrD[i+1];
    for (j = pointrD[i]; j < end; j++) 
      temp[i] += valueD[j] * x[rowindD[j]];
  }

  memset(y, 0, n * sizeof(double));

  for (i = 0; i < matX->cols; i++) {//y = X*temp
    end = pointrX[i+1];
    for (j = pointrX[i]; j < end; j++) 
      y[rowindX[j]] += valueX[j] * (temp[i]);
  }

  for (i = 0; i < matD->rows; i++) temp[i] = 0.0;

  for (i = 0; i < matD->cols; i++) {//temp = D*y
    end = pointrD[i+1];
    for (j = pointrD[i]; j < end; j++) 
      temp[rowindD[j]] += valueD[j] * y[i];
  }

  memset(y, 0, n * sizeof(double));

  for (i = 0; i < matD->cols; i++) {//y = D.T*temp
    end = pointrD[i+1];
    for (j = pointrD[i]; j < end; j++) 
      y[i] += valueD[j] * temp[rowindD[j]];
  }

  for (i = 0; i < matD->rows; i++) temp[i] = 0.0;

  for (i = 0; i < matX->cols; i++) {//temp = X.T*y
    end = pointrX[i+1];
    for (j = pointrX[i]; j < end; j++) 
      temp[i] += valueX[j] * y[rowindX[j]];
  }

  memset(y, 0, n * sizeof(double));

  for (i = 0; i < matD->cols; i++) {//y = D*temp
    end = pointrD[i+1];
    for (j = pointrD[i]; j < end; j++) 
      y[rowindD[j]] += valueD[j] * temp[i];
  }
  /*for (i = 0; i < A->cols; i++) {// A*x
    end = pointr[i+1];
    for (j = pointr[i]; j < end; j++) 
      temp[rowind[j]] += value[j] * (*x); //one element in vector x multiply one column in matrix A
    x++;
  }
  for (i = 0; i < A->cols; i++) {//A'*(Ax)
    end = pointr[i+1];
    for (j = pointr[i]; j < end; j++) 
...
/***********************************************************
 * multiplication of matrix DXD.T by vector x, where A is 	   *
 * nrow by ncol (nrow >> ncol).  y stores product vector.  *
 ***********************************************************/
void svd_opa(SMat matX, SMat matD, double *x, double *y, double *temp) {
  long end, i, j;
  long *pointrX = matX->pointr, *rowindX = matX->rowind;
  double *valueX = matX->value;
  ...
  memset(y, 0, matD->rows * sizeof(double));

  for (i = 0; i < matD->cols; i++) {//y = D.T*x
    end = pointrD[i+1];
    for (j = pointrD[i]; j < end; j++) 
      y[i] += valueD[j] * x[rowindD[j]];
  }

  for (i = 0; i < matD->rows; i++) temp[i] = 0.0;

  for (i = 0; i < matX->cols; i++) {//temp = X*y
    end = pointrX[i+1];
    for (j = pointrX[i]; j < end; j++) 
      temp[rowindX[j]] += valueX[j] * (y[i]);
  }
