...
void   ortbnd(double *alf, double *eta, double *oldeta, double *bet, long step,
              double rnm);
double startv(SMat matX, SMat matD, double *wptr[], long step, long n);
...
void   write_header(long, long, double, double, long, double, long, long,
                    long);
long   check_parameters(SMat matX, SMat matD, long dimensions, long iterations,
                        double endl, double endr, long vectors);
int    lanso(SMat matX, SMat matD, long iterations, long dimensions, double endl,
             double endr, double *ritz, double *bnd, double *wptr[],
             long *neigp, long n);
long   ritvec(long n, SMat matX, SMat matD, SVDRec R, double kappa, double *ritz,
              double *bnd, double *alf, double *bet, double *w2,
              long steps, long neig);
long   lanczos_step(SMat matX, SMat matD, long first, long last, double *wptr[],
                    double *alf, double *eta, double *oldeta,
                    double *bet, long *ll, long *enough, double *rnmp,
                    double *tolp, long n);
void   stpone(SMat matX, SMat matD, double *wrkptr[], double *rnmp, double *tolp, long n);
...
long check_parameters(SMat matX, SMat matD, long dimensions, long iterations,
   ...
   else if (matD->rows <= 0 ) error_index = 4;
   /*else if (n > A->cols || n > A->rows) error_index = 1;*/
   else if (iterations <= 0 || iterations > matD->rows)
     error_index = 5;
...
SVDRec svdLAS2A(SMat matX, SMat matD, long dimensions) {
  ...
  return svdLAS2(matX, matD, dimensions, 0, end, kappa);
}


SVDRec svdLAS2(SMat matX, SMat matD, long dimensions, long iterations, double end[2],
               double kappa) {
  ...
  /* If DXD.T is wide, the SVD is computed on its transpose for speed.
    however, it's imposssible.*/
  //if (D->rows >= D->rows * 1.2) {
  ...
  if (!(LanStore = (double **) calloc(iterations + MAXLL, sizeof(double *))))
    goto abort;
  if (!(OPBTemp = svd_doubleArray(matD->rows, FALSE, "las2: OPBTemp")))
    goto abort;

  /* Actually run the lanczos thing: */
  steps = lanso(matX, matD, iterations, dimensions, end[0], end[1], ritz, bnd, wptr,
                &neig, n);
  ...
  R->Ut = svdNewDMat(R->d, matD->rows);
  R->S  = svd_doubleArray(R->d, TRUE, "las2: R->s");
  R->Vt = svdNewDMat(R->d, matD->rows);
  if (!R->Ut || !R->S || !R->Vt) {
    svd_error("svdLAS2: allocation of R failed");
    goto cleanup;
  }

  nsig = ritvec(n, matX, matD, R, kappa, ritz, bnd, wptr[6], wptr[9], wptr[5], steps,
                neig);
...
long ritvec(long n, SMat matX, SMat matD, SVDRec R, double kappa, double *ritz, double *bnd,
  ...
  if (ierr) {
    ...
    for (x = 0; x < R->d; x++) {
      /* multiply by matrix DX.TD.T DXD.T first */
      svd_opb(matX, matD, R->Vt->value[x], xv2, OPBTemp);
      tmp0 = svd_ddot(n, R->Vt->value[x], 1, xv2, 1);
      svd_daxpy(n, -tmp0, R->Vt->value[x], 1, xv2, 1);
      tmp0 = sqrt(tmp0);
      xnorm = sqrt(svd_ddot(n, xv2, 1, xv2, 1));

      /* multiply by matrix DXD.T to get (scaled) left s-vector */
      svd_opa(matX, matD, R->Vt->value[x], R->Ut->value[x], OPBTemp);
      tmp1 = 1.0 / tmp0;
      svd_dscal(matD->rows, tmp1, R->Ut->value[x], 1);
      xnorm *= tmp1;
      bnd[i] = xnorm;
      R->S[x] = tmp0;
    }
...
int lanso(SMat matX, SMat matD, long iterations, long dimensions, double endl,
  ...
  long ll, first, last, ENOUGH, id1, id2, id3, i, l, neig, j = 0, intro = 0;
  ...
  /* take the first step */
  stpone(matX, matD, wptr, &rnm, &tol, n);
  ...
  while (/*id1 < dimensions && */!ENOUGH) {
    ...
    j = lanczos_step(matX, matD, first, last, wptr, alf, eta, oldeta, bet, &ll,
                     &ENOUGH, &rnm, &tol, n);
    ...
    *neigp = neig;
    ...
    id1++;
    /*printf("end of id1=%ld dimen=%ld first=%ld\n", id1, dimensions, first);*/
  }
...
long lanczos_step(SMat matX, SMat matD, long first, long last, double *wptr[],
   ...
   for (j=first; j<last; j++) {
      ...
      svd_opb(matX, matD, wptr[3], wptr[0], OPBTemp);
...
void stpone(SMat matX, SMat matD, double *wrkptr[], double *rnmp, double *tolp, long n) {
   ...
   rnm = startv(matX, matD, wrkptr, 0, n);
   ...
   /* take the first step */
   svd_opb(matX, matD, wrkptr[3], wrkptr[0], OPBTemp);
...
long error_bound(long *enough, double endl, double endr,
  ...
  for (i=((step+1) + (step-1)) / 2; i >= mid + 1; i -= 1) {
    /*printf("gap=%E eps34=%E reps=%E fabs=%E\n",
      fabs(ritz[i-1] - ritz[i]), eps34, reps, fabs(ritz[i]));*/
